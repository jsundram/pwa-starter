...
// The skeleton's shell as the tests need it: the root document boots scriptless (its BOOT_DEPS
// entry is empty), so usage/ — whose dashboard dies without crunch.js — is the bootability case.
const seedBootableShell = () => {
  CACHE.set(BASE, makeResponse("CACHED_ROOT"));
  CACHE.set(b("index.html"), makeResponse("CACHED_INDEX"));
  CACHE.set(b("usage/"), makeResponse("CACHED_USAGE"));
  CACHE.set(b("usage/index.html"), makeResponse("CACHED_USAGE_INDEX"));
...
(async () => {
  ...
  ok("cached subresource → cache, 0 fetches", bodyOf(r) === "CACHED_APP" && fetchCalls === 0, `fetches=${fetchCalls}`);

  reset("slow"); seedBootableShell();
  r = await start(req(b("usage/"), "navigate"));
  ok("usage/ nav → its OWN page (not index)", bodyOf(r) === "CACHED_USAGE" && fetchCalls === 0);
  ...
  reset("slow");
  CACHE.set(b("usage/"), makeResponse("CACHED_USAGE"));   // doc cached, crunch.js absent → not bootable
  p = start(req(b("usage/"), "navigate"));
  await tick(2999);
  ...
  reset("ok", 500);
  CACHE.set(b("usage/"), makeResponse("CACHED_UNBOOTABLE_USAGE"));   // cached but crunch.js absent
  r = await start(req(b("usage/"), "navigate"));
  ok("nav + server 500 → honest fallback, not bare doc (issue 2)", r && r.status === 503 && bodyOf(r) !== "CACHED_UNBOOTABLE_USAGE", `status=${r && r.status} body=${bodyOf(r)}`);
  ...
  reset("offline-heal");
  CACHE.set(b("usage/crunch.js"), makeResponse("CACHED_CRUNCH"));      // so bootable() passes in the catch
  healEntry = [b("usage/"), makeResponse("REPAIRED_USAGE")];   // "ensure-shell" repairs during the fetch
  r = await start(req(b("usage/"), "navigate"));
  ok("catch re-reads cache → serves mid-window repair (issue 3)", bodyOf(r) === "REPAIRED_USAGE", `body=${bodyOf(r)}`);

  // --- skeleton-specific branches -------------------------------------------
  // Precached JSON (manifest.json is in SHELL) is served without revalidation: the refresh would
  ...
  // Google Fonts branch: cache-first, so type survives offline.
  reset("slow");
  CACHE.set("https://fonts.gstatic.com/s/font.woff2", makeResponse("CACHED_FONT"));
  r = await start(req("https://fonts.gstatic.com/s/font.woff2"));
  ok("cached webfont → cache, 0 fetches", bodyOf(r) === "CACHED_FONT" && fetchCalls === 0, `fetches=${fetchCalls}`);
